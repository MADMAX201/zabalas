...
- **Núcleo familiar / acompañantes:** cada integrante gestiona en `/perfil#familia` su lista (`household_members`: nombre + parentesco). Al confirmar "Sí, voy" marca quiénes van (`rsvp_companions`) y puede sumar "otros acompañantes" (`rsvps.extra_guests`); `rsvps.guests` = marcados + extras (total). Se muestran por nombre en "Quiénes van", en admin y en el Excel (columna "Quiénes"). En la tienda, "Para quién" sugiere esos nombres (datalist).
...
`users` (role member|admin, active) · `settings` (family_code, site_name, nequi_number, nequi_holder, nequi_qr, payment_instructions) · `events` · `event_dates` · `rsvps` (yes|no|maybe, guests, extra_guests) · `date_rsvps` · `household_members` · `rsvp_companions` · `products` (sizes "S,M,L", stock NULL=∞, order_deadline) · `orders` (status pending→review→paid|rejected, cancelled; receipt, receipt_ref) · `order_items` (size, qty, for_name, unit_price).
...
- 2026-09-07: v1.5 — núcleo familiar en el perfil y selección por nombre de quiénes van contigo al confirmar asistencia; nombres visibles para todos, admin y Excel; sugerencia de nombres en 'Para quién' de la tienda.
